...
print(df_nino)
signal = df_nino.values.squeeze() # массив из элементов
print('-------------')
print(signal)
print(len(signal))
...
def plot_signal_plus_average(ax, time, signal, average_over = 5):
    ...
    ax.plot(time_ave, signal_ave, label = 'Обработанный сигнал (n={})'.format(average_over))
    ax.set_xlim([time[0], time[-1]])
    ax.set_ylabel('f(x)', fontsize=16)
    ax.set_title('Функция', fontsize=16)
    ax.legend(loc='upper right')


fig, ax = plt.subplots(figsize=(12,3))
plot_signal_plus_average(ax, time, signal, average_over = 5)
plt.show()
...
def plot_fft_plus_power(ax, time, signal, plot_direction='horizontal', yticks=None, ylim=None):
    ...
    if plot_direction == 'horizontal':
        ax.plot(f_values, fft_values, 'r-')
        ax.plot(f_values, fft_power, 'k--', linewidth=1)
    elif plot_direction == 'vertical':
        scales = 1. / f_values
        scales_log = np.log2(scales)
        ax.plot(fft_values, scales_log, 'r-')
        ax.plot(fft_power, scales_log, 'k--', linewidth=1)
...
def plot_wavelet(ax, time, signal, scales, waveletname='cmor', cmap=plt.cm.seismic, title='', ylabel='', xlabel=''):
    dt = time[1] - time[0]
